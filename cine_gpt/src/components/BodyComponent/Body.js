...
const Body = () =>{
    ...
    return (
        <div className="Body">
            <form onClick={(e)=>e.preventDefault()} className="w-3/12 h-3/4 bg-black bg-opacity-75 my-6 mx-auto p-6 flex flex-col space-y-11 rounded-lg shadow-lg">
                <h1 className="text-white text-3xl">{isSignUp?"Sign Up":"Sign In"}</h1>
                {isSignUp &&<input
                ...
                <p onClick={handleFormChange} className="text-white cursor-pointer">{isSignUp?"Already a user? Sign In":"New to Netflix? Sign Up Now"}</p>
            </form>
        </div>
    );
}
